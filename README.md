...
`docker-compose.yml` exposes Postgres on host port **5433**, not the usual 5432 — avoids silently
colliding with a Postgres already installed on the machine (e.g. a Homebrew service): `localhost`
can resolve to that one instead of the container depending on IPv4/IPv6 resolution order, which is
very confusing to debug (queries and migrations appear to hit an empty DB while actually landing
elsewhere). The `default` Spring profile's datasource URL (`application.yml`) matches on 5433.

If you'd rather use a Postgres already installed on your machine instead of Docker, create the
matching role/database with:
...
...then either have that instance listen on 5433 instead of its default 5432, or override
`SPRING_DATASOURCE_URL` (see "Environment variables" below) to point at whatever port it uses.
...
(`/admin` in the frontend, or the admin API below), or — for bulk content supplied as `.docx` documents,
as with AZ-900 (`V2`-`V15`) and AZ-104 (`V18`-`V30`) — a dedicated Flyway migration generated from it.
...
- **A dedicated Flyway migration, for bulk `.docx`-sourced content** — the original path for AZ-104's Review
  Questions and Practice Scenarios (`V18`-`V30`, 820 questions across 13 modules covering all six topics —
  Compute, Identities & Governance, Storage, Virtual Networks, Monitoring, Complementary Topics — plus a
  French-language revision set), generated by `scripts/generate_az104_content_migrations.py` from paired
  "Questions"/"AnswerKey" `.docx` documents (stdlib-only `.docx` parsing via `zipfile`/`ElementTree`, no
  `pandoc`/`python-docx` dependency). Same rationale as `V2`-`V15` below: this content ships baked into the
  migrations so it's present the moment the app boots against a fresh database, in any environment, with no
  manual import step to remember. Not used for image-heavy content like mock exams, which go through the
  admin UI instead. Run the generator with `--dry-run` first — it parses, cross-validates each question
  against its answer key, and reports without writing any file. See the script's own module docstring for
  the full design rationale, in particular: **it deliberately reshuffles every question's option order
  before import.** The source `.docx` files put the correct answer at a heavily skewed or literally fixed
  position (e.g. the Compute review set has the correct answer as option A in 75 out of 75 questions; option
  D is never once correct across any of the six review sets) — importing as-is would let a student ace the
  quiz by guessing position instead of knowing the material.
...
- Sourcing question content itself (supplied separately) — see "Content authoring" above for how it's
  imported once supplied, whether by hand through `/admin` or via a generated Flyway migration.
